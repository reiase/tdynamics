...
from .step import *
from .plot import LazyPlot as L
from .plot import plot


def FLAT(x):
    return x.to(torch.float32).flatten().cpu()


def linear(layer_or_weight: torch.nn.Linear, input: torch.Tensor, name: str = "Linear"):
    if isinstance(layer_or_weight, torch.nn.Linear):
        layer = layer_or_weight
        weight = layer.weight
    else:
        layer = None
        weight = layer_or_weight
    ...
    figs = [
        [
            L.hist(input.FLAT(), bins=100).title("i/ hist"),
        ],
    ]

    return inspect.linear(layer, input, name, weight)
